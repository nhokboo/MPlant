...
function ($document, PanZoomService) {
    ...
    function ($scope, $element, $timeout) {
                        ...
                        var syncModelToDOM = function () {
                            ...
                            //editting by nam.nguyen@mimosatek.com
                        ...
                        var changeZoomLevel = function (newZoomLevel, clickPoint, duration) {
                            ...
                            var translationFromZoom = function (zoomLevel) {
                                ...
                                //var x_temp = t1.x - t0.x;
                                //var y_temp = t1.y - t0.y;
                                //alert(x_temp);
                                return {
                                    x: t1.x - t0.x,
                                    y: t1.y - t0.y
                                ...
                                if ($scope.panVelocity) {
                                    ...
                                    while (deltaTime > 0) {
                                        ...
                                        if (speed < $scope.config.haltSpeed) {
                                            //$scope.panVelocity = undefined;
                                            $scope.panVelocity.x = 0;
                                            $scope.panVelocity.y = 0;
                                            $scope.config.modelChangedCallback($scope.model);
                        ...
                        var pan = function (delta) {
                            delta.x = delta.x || 0;
                            delta.y = delta.y || 0;
                            //edited by nam.nguyen@mimosatek.com
                            if($scope.base.pan.x + delta.x <0 && $scope.base.pan.y + delta.y<0){
                                var tempx = $scope.base.pan.x + delta.x +$element.width()*$scope.scale;
                                var tempy = $scope.base.pan.y + delta.y + $element.height()*$scope.scale;
                                //console.log('height= ' + $element.height());
                                //console.log('y= ' + $scope.base.pan.y);
                                //console.log('deltaY=' + delta.y);
                                //console.log('scale=' + $scope.scale);
                                //console.log('tempy= ' + tempy);
                                //console.log('------------------');
                                if(tempx  > $element.width() && tempy > $element.height()){

                                    $scope.base.pan.x += delta.x;
                                    $scope.base.pan.y += delta.y;
                                }else if (tempx  > $element.width() && tempy < $element.height() ){

                                    $scope.base.pan.x += delta.x;
                                    $scope.base.pan.y += delta.y+($element.height() - tempy);
                                }else if(tempx  < $element.width() && tempy > $element.height()){
                                    $scope.base.pan.y += delta.y;
                                    $scope.base.pan.x += delta.x+($element.width() - tempx);
                                }

                            }
                            //else if($scope.base.pan.x + delta.x <0 && $scope.base.pan.y + delta.y >0)
                            //{
                            //    $scope.base.pan.y = 0;
                            //}else if($scope.base.pan.x + delta.x >0 && $scope.base.pan.y + delta.y <0){
                            //
                        ...
                        $scope.onMousemove = function ($event) {
                            //if($scope.config.panOnClickDrag || (!$scope.config.panOnClickDrag && !$scope.config.zoomOnMouseWheel))
                            {
                            var now = jQuery.now();
                        ...
                        $scope.onMouseup = function () {
                            ...
                            try {
                                if ($scope.panVelocity) {
                                    ...
                                    console.log(initialMultiplier );
                                    console.log($scope.panVelocity.x);
                                    console.log('------------------');
                                }
                            }catch(e){
                                $scope.panVelocity.x = 0;
                        ...
                        $scope.onMouseWheel = function ($event, $delta, $deltaX, $deltaY) {
                            if ($scope.config.zoomOnMouseWheel) {
                                ...
                                var clickPoint = {
                                    ...
                                    //x: $element.width()/2 - frameElement.offset().left,
                                    //y: $element.height()/2 - frameElement.offset().top
                                };
